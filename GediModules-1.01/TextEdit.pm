...
use vars qw($VERSION);
$VERSION = '3.005'; # $Id: //depot/Tk8/Tk/TextEdit.pm#4 $
...
# given a set of selected lines.
# figure out if any characters at start of lines are a space or tab.
# first line, first whitespace, is returned.
# This can be used by indent to follow whatever indenting method is
# already being used by user.
# once its detected, store it in Object. If a sub is called and no
# whitespace exists at start of any lines, use the last type of whitespace

use Data::Dumper;
sub FirstWhiteSpaceAtStartOfSelectedLines
{
	my ($w)=@_;
	my %lines = $w->tagRanges('sel');
	my @lines;
	while(my($start,$end)=each(%lines))
		{
		$start=~m{^(\d+)};
		my $start_line=$1;
		$end=~m{^(\d+)};
		...
		for (my $ln=$start_line; $ln<=$end_line; $ln++)
			{
			my $index = $ln.'.0';
			...
			if( $char =~ m{([\ \t])} )
				...
				$w->{'INDENT_STRING'}=$whitespace; 
				last;
				}
			}
		}
	return $w->{'INDENT_STRING'};
...
{
 my($w)=@_;
 $w->insertStringAtStartOfSelectedLines($w->FirstWhiteSpaceAtStartOfSelectedLines);
}

sub UnindentSelectedLines
{
 my($w)=@_;
 $w->deleteStringAtStartOfSelectedLines($w->FirstWhiteSpaceAtStartOfSelectedLines);
}
